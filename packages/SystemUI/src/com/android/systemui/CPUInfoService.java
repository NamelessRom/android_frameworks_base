...
public class CPUInfoService extends Service {
    ...
    private class CPUView extends View {
        ...
        CPUView(Context c) {
            ...
            final String maxWidthStr = "cpuX xxxxxxxxxxxxxx 1700000 MHz";
        ...
        public void run() {
            try {
                while (!mInterrupt) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        String currFreq = readOneLine(CPU_ROOT + i + CPU_CUR_TAIL);
                        String currGov = readOneLine(CPU_ROOT + i + CPU_GOV_TAIL);
